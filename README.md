...
cryptographic check) currently reaches an `assert_eq!` inside winterfell's
`Air::new` and traps the whole call. This is safe — a trapped `update` call
is atomic and reverts cleanly, it does not corrupt canister state or accept
...
returned. Worth hardening later, e.g. wrapping the untrusted-input path in
...
proof itself grows less than 3×. That sublinear relationship is the reason
this architecture is viable at all — proving cost grows with trace length,
but verification cost barely does.

Instruction counts came from `ic_cdk::api::instruction_counter()`,
...
execution environment and charges cycles against the same fee schedule —
it is not a mock. It is *not*, however, IC mainnet: no real ICP was spent,
there's no real subnet of independent node providers reaching consensus,
and mainnet cycle prices/exchange rates can differ slightly from what a
local replica assumes. Actually deploying to mainnet needs a cycles wallet
funded from your own ICP, under your own identity — that's a `dfx deploy
--network ic` away once you're happy with local results, but it's a step
only you can authorize and fund.
...
- Very large proofs (many queries / high blowup factor, used for
  ...
  logarithmic-ish in trace length, so this scales far better than proving
  would, but it isn't literally free — benchmark with realistic proof sizes
  for your computation before assuming any given security level is cheap
  enough for your cycle budget.
